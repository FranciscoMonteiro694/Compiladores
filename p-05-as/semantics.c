...
int check_integer_dec(is_integer_dec* iid) {
    ...
    if(newel==NULL) {
        printf("Symbol %s already defined! Linha:%d Coluna:%d \n", iid->id,iid->linha,iid->coluna);
        return 1;
    }
    return 0;
}
...
int check_write_statement(is_write_statement* iws) {
    ...
    if(aux==NULL) {
        printf("Symbol %s not declared! Linha:%d Coluna:%d \n", iws->id,iws->linha,iws->coluna);
        return 1;
    }
    return 0;
}
